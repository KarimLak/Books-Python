...
def click_all_links(driver, button_texts):
    ...
    for button_text in button_texts:
        ...
        for index, link in enumerate(zoom_demo_links):
            href = link.get('href')
            print(href)
            book_data = scrape_book(href, button_text)  # Pass button_text as the award argument
...
def scrape_book(url, award):
    ...
    book_data = {
        ...
        'description': (award_string[1:] if award_string.startswith(' ') and 'Lauréat' not in award_string else award_string) + ' : Roman-Nouvelles-Récit 2023',
    }

    return book_data


url = 'https://prixdeslibraires.qc.ca/categorie-roman-selection/'
button_texts = ['Préliminaire 2023', 'Finalistes Roman-Nouvelles-Récit 2023', 'Lauréats Roman-Nouvelles-Récit 2023']
...
with open("output-prixdeslibraires-roman.ttl", "w", encoding="utf-8") as f:
