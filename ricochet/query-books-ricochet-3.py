...
def extract_first_year(year_range):
    if year_range:
        if '-' in year_range:
            return year_range.split('-')[0].strip()
        else:
            return year_range[:4].strip()
    else:
        return None
        ...
        if laureates_header:
            ...
            if laureates_content:
                ...
                for entry in entries:
                    ...
                    year_line = re.search(r'<strong>(\d+)|<b>(\d+)', entry)
                    if year_line:
                        year = year_line.group(1) or year_line.group(2)
...
all_award_links = ['/prix-litteraires/prix-du-roman-historique-jeunesse']
...
turtle_output = g.serialize(format="turtle")
with open("output-ricochet-tables-3.ttl", "w", encoding="utf-8") as f:
