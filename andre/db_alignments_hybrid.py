...
class InterDBStats:
    ...
    def increment_collision_number_exact_key(self):
        self.collision_number_exact_key += 1
    ...
    def align_by_key_isbn(self, book_alignment, book_key):  # True if alignment in this call or already done (collision)
    ...
    def align_by_key(self, book_alignment,
                     book_key_to_match):  # True if alignment in this call or already done (collision)
...
with Parallel(n_jobs=N_JOBS) as parallel:
    for book in g.subjects(RDF.type, ns1.Book):  # O(M)
        ...
        start = time.time()
        stats.align_hybrid(copy.deepcopy(book_alignment_bnf), name_author_date_key)
        end = time.time()
